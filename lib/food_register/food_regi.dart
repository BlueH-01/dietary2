...
<<<<<<< HEAD
  Widget _buildInputField(String labelText, TextEditingController controller,
    ...
    return Padding(
      padding: const EdgeInsets.only(bottom: 12.0), // 아래쪽 여백 추가
=======
  // 입력 필드 생성
  Widget _buildInputField(String labelText, TextEditingController controller,
      {bool isNumeric = false, IconData? icon}) {
...
>>>>>>> f3c9626 (조민기)
      child: TextField(
        ...
        decoration: InputDecoration(
          labelText: labelText,
<<<<<<< HEAD
          labelStyle: const TextStyle(color: Colors.grey), // 라벨 글씨 색
          hintText: '입력하세요',
          hintStyle: const TextStyle(color: Colors.grey), // 힌트 텍스트 색
          ...
          contentPadding: const EdgeInsets.symmetric(
              vertical: 8, horizontal: 12), // 위아래 간격 조정
        ),
      ),
    );
  }
...
Widget build(BuildContext context) {
 ...
 Future<List<DocumentSnapshot>> _sortFavoritesFirst(
  ...
  for (final foodDoc in foodList) {
    final isFavorite = await _isFavorite((foodDoc.data() as Map<String, dynamic>)['name']); // 음식 이름으로 즐겨찾기 여부 확인
    if (isFavorite) {
